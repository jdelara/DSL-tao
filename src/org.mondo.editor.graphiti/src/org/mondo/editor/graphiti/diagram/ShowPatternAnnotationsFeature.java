 ...
 *
 */
public class ShowPatternAnnotationsFeature extends AbstractCustomFeature {
 
    private boolean hasDoneChanges = false;
      
    public ShowPatternAnnotationsFeature(IFeatureProvider fp) {
        super(fp);
    }
 
    @Override
    public String getName() {
        return "Show Pattern Annotations";
    }
 
    @Override
    public String getDescription() {
        return "Show annotations related to patterns";
    }
    ...
    public void execute(ICustomContext context) {    
        ...
        if (bo instanceof EPackage) {
        			...
        			if (pe instanceof ContainerShape){
    					if (eClassif instanceof EClass) {
    						if (DiagramUtils.isExpand((ContainerShape)pe)){
	    						DiagramUtils.showAnnotationsEClass((ContainerShape)pe, getFeatureProvider());
	    						hasDoneChanges = true;
	    						for (EReference eRef : ((EClass) eClassif).getEReferences()){
	    	          				DiagramUtils.showAnnotationsEReference(getDiagram(), eRef);
	    	          			}
	    						break;
    						}
    					}
        	...
        	DiagramUtils.setShowAnnotations(getDiagram(), true);
        }
    }
